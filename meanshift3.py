...
def meanShiftwindow2(frame, track_window, q, weights, epsilon, max_iterations, window_size):
    ...
    while True:
        ...
        hsv_roi = cv2.resize(hsv_roi, (w // window_size, h // window_size))
        ...
        x_change = np.sum(average_X_mat) / np.sum(weights_mat) * window_size
...
def meanShiftTracking3(cap, file_name, ROI, kernel, gamma, window_size, occlusion = False):
    ret, frame = cap.read()
    if file_name != False: 
    ...
    weights = kernel((h//window_size, w//window_size))
    rgb_roi = frame[y:y+h, x:x+w]
    hsv_roi = cv2.cvtColor(rgb_roi, cv2.COLOR_BGR2HSV)
    hsv_roi = cv2.resize(hsv_roi, (w // window_size, h // window_size))
    ...
    while cap.isOpened():
        ...
        if ret == True:
            before = time.time()
            x,y,w,h = meanShiftwindow2(frame, (x,y,w,h), q, weights, 1, 10, window_size)
            after = time.time() 
...
if __name__=="__main__":
    ...
    gamma = 0.6
    window_size = 1
    meanShiftTracking3(cap, file_name, ROI, epanechnikov_kernel, gamma, window_size)
