...
def meanShiftBPW(frame, track_window, q, weights, epsilon, max_iterations):
    ...
    while True:
        ...
        p = normalize_histogram(extract_histogram(hsv_roi, 8))
        v = np.sqrt(np.divide(q, p + 1e-5))
        weights_mat = backproject_histogram(hsv_roi, v, 8)
        weights_mat = bpw(weights_mat, weights, 8)
    ...
    return x,y, w,h
def meanShiftTrackingBPW(cap, file_name, ROI, kernel, gamma, occlusion = False):
    ret, frame = cap.read()
    if file_name != False: 
    ...
    while cap.isOpened():
        ...
        if ret == True:
            before = time.time()
            x,y,w,h = meanShiftBPW(frame, (x,y,w,h), q, weights, 1, 10)
            after = time.time() 
...
if __name__=="__main__":
    ...
    file_name =  "./results_video/original_meanshift_f.avi"
    ROI = (3100, 1550, 200, 200)
    gamma = 0.6
    occlusion = 0.2
    time, _ = meanShiftTrackingBPW(cap, file_name, ROI, epanechnikov_kernel, gamma, occlusion)
